...
-- utf8.graphemes: iterate over grapheme clusters (simplified: one UTF-8 codepoint per cluster)
...
do
    ...
    table.freeze = function(t_)
        if type(t_) == "table" then
            ...
            -- Move all existing keys to a backing store and redirect via __index.
            -- This is necessary because __newindex is only called for NEW keys;
            -- existing keys bypass it. By removing them from t_ and putting them
            -- in a backing store, all reads/writes go through metamethods.
            local existing_mt = _rawget and _rawget(getmetatable(t_), "__newindex")
            if not existing_mt then  -- not already frozen
                local backing = {}
                for k, v in next, t_ do
                    _rawset(backing, k, v)
                    _rawset(t_, k, nil)
                end
                local freeze_mt = {}
                _rawset(freeze_mt, "__index",    backing)
                _rawset(freeze_mt, "__newindex", function(_, k, v)
                    error("attempt to modify a frozen table", 2)
                end)
                _rawset(freeze_mt, "__len",   function() return #backing end)
                _rawset(freeze_mt, "__pairs", function() return next, backing, nil end)
                _setmetatable(t_, freeze_mt)
            end
        end
        return t_
    end
...
function q.dump_file(eN, eO)
    ...
    do
        ...
        _table_ext.freeze = function(t_)
            if type(t_) == "table" then
                _frozen[t_] = true
                -- Move all existing keys to a backing store so __newindex is triggered on writes.
                -- __newindex is only called for NEW keys; for existing keys Lua does a direct rawset.
                local _existing_mt = a.getmetatable(t_)
                local _already = _existing_mt and (type(_existing_mt) == "table") and _rawget(_existing_mt, "__newindex")
                if not _already then
                    local _backing = {}
                    for k, v in next, t_ do
                        _rawset(_backing, k, v)
                        _rawset(t_, k, nil)
                    end
                    local _freeze_mt = {}
    ...
    do
        ...
        if _tbl and _tbl.freeze then
            local function _try_freeze(t_)
                if type(t_) == "table" then pcall(_tbl.freeze, t_) end
            end
