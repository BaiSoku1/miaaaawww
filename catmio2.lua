...
function q.dump_file(eN, eO)
    ...
    rawset(eR, "hookmetamethod",       function(obj, m_, r_)
        -- Return the ORIGINAL metamethod so callers can save and invoke it
        local _orig = nil
        if obj ~= nil then
            local _mt = (type(obj)=="table" or type(obj)=="userdata") and a.getmetatable(obj)
            if _mt and type(m_) == "string" then
                _orig = _mt[m_]
            end
        end
    ...
    rawset(eR, "hookmetamethod",  function(obj, method, hook)
        ...
        if obj ~= nil then
            local _mt = (type(obj)=="table" or type(obj)=="userdata") and a.getmetatable(obj)
            if _mt and type(method) == "string" then _orig = _mt[method] end
        end
        if type(_orig) ~= "function" then _orig = function() end end
        return _orig
    end)
