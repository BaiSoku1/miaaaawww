...
local r = {
    ...
    MEM_LIMIT_KB = 512 * 1024,  -- 512 MB hard cap on Lua heap; OOM scripts get killed early
    MAX_STRING_REP_BYTES = 10 * 1024 * 1024,  -- 10 MB max per string.rep call in sandbox
    MAX_TABLE_ENTRIES = 2000000,  -- max entries any single sandbox table may hold
...
-- Cap string.rep globally to prevent OOM from malicious scripts
...
string.rep = function(s, n, sep)
    ...
    local _est = _unit * _n
    local _MAX = 256 * 1024 * 1024  -- 256 MB global cap
    if _est > _MAX then
        local _safe_n = math.max(1, math.floor(_MAX / math.max(1, _unit)))
        return _orig_string_rep(_s, _safe_n, _sep)
    end
    return _orig_string_rep(_s, _n, _sep)
end
...
function q.dump_file(eN, eO)
    ...
    -- string override: cap string.rep to prevent memory exhaustion attacks
    do
        ...
        _string_ext.rep = function(s, n, sep)
            ...
            if _est > r.MAX_STRING_REP_BYTES then
                -- Return a truncated safe version instead of crashing
                local _safe_n = math.max(1, math.floor(r.MAX_STRING_REP_BYTES / math.max(1, _unit)))
                return string.rep(_s, _safe_n, _sep)
            end
            return string.rep(_s, _n, _sep)
        end
