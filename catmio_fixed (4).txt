...
local r = {
    ...
    MEM_LIMIT_KB = 256 * 1024,  -- 256 MB hard cap on Lua heap; OOM scripts killed early
    MAX_STRING_REP_BYTES = 1 * 1024 * 1024,  -- 1 MB max per string.rep call in sandbox
    MAX_EXEC_TOTAL_ALLOC = 50 * 1024 * 1024,  -- 50 MB cumulative string.rep budget per execution
    MAX_TABLE_ENTRIES = 500000,  -- max entries any single sandbox table may hold
...
local function I(J)
    ...
    local function Q(R)
        ...
        -- Luau inline if-expression: "= if cond then a else b" → "= (cond) and (a) or (b)"
        -- Pattern: assignment or return followed by inline if-expression on same line
        R = R:gsub("=%s*if%s+(.-)%s+then%s+(.-)%s+else%s+(.-)%s*$", function(cond, a, b)
...
bj = function(aQ, bO, bw)
    ...
    bP.GetService = function(self, bQ)
        ...
        if bR == "DebuggerManager" then
            error("GetService: DebuggerManager is not accessible", 2)
        end
        -- Return cached proxy so same-service calls return the SAME object
        if not t.service_cache then t.service_cache = {} end
        if t.service_cache[bR] then
            return t.service_cache[bR]
        end
        ...
        if bR == "LocalizationService" then
            t.property_store[x].RobloxLocaleId = "en-us"
            t.property_store[x].SystemLocaleId = "en-us"
        end
        if bR == "Lighting" then
            local _techItem = bj("Enum.Technology.Future", false)
            t.registry[_techItem] = "Enum.Technology.Future"
            t.property_store[_techItem] = {Name = "Future", Value = 4, EnumType = bj("Enum.Technology", false)}
            t.registry[t.property_store[_techItem].EnumType] = "Enum.Technology"
            t.property_store[x].Technology = _techItem
            t.property_store[x].ClockTime = 14
            t.property_store[x].Brightness = 1
            t.property_store[x].GlobalShadows = true
        end
        if bR == "SoundService" then
            t.property_store[x].RespectFilteringEnabled = false
            t.property_store[x].VolumetricAudio = false
        end
        -- Cache so repeated GetService calls return the SAME proxy
        if not t.service_cache then t.service_cache = {} end
    ...
    bP.WaitForChild = function(self, bT, bU)
        local bV = aE(bT)
        -- Error for negative timeout (real Roblox behavior)
        if bU ~= nil then
            local _to = tonumber(bU)
            if _to and _to < 0 then
                error("bad argument #2 to 'WaitForChild' (timeout must be non-negative)", 2)
            end
        end
    ...
    bP.Destroy = function(self)
        ...
        -- Remove from parent's children list and set Parent to nil
        local _prev_parent = t.parent_map[bh]
        if _prev_parent and t.children_map and t.children_map[_prev_parent] then
            for _i, _ch in ipairs(t.children_map[_prev_parent]) do
                if _ch == bh then table.remove(t.children_map[_prev_parent], _i); break end
            end
        end
        t.parent_map[bh] = nil
        t.property_store[bh] = t.property_store[bh] or {}
        t.property_store[bh].Parent = nil
    end
    ...
    bP.Connect = function(self, bs)
        -- Register GetPropertyChangedSignal callback
        if t.property_store[bh] and t.property_store[bh]._signal_instance then
            local _si = t.property_store[bh]._signal_instance
            local _sp = t.property_store[bh]._signal_property
            t._changed_callbacks = t._changed_callbacks or {}
            t._changed_callbacks[_si] = t._changed_callbacks[_si] or {}
            t._changed_callbacks[_si][_sp] = t._changed_callbacks[_si][_sp] or {}
            if type(bs) == "function" then
                table.insert(t._changed_callbacks[_si][_sp], bs)
            end
        end
        local bS = t.registry[bh] or "signal"
        local c1 = bj("connection", false)
        -- Mark connection as active immediately
        t.property_store[c1] = t.property_store[c1] or {}
        t.property_store[c1].Connected = true
        local c2 = aW(c1, "conn")
    ...
    bP.Disconnect = function(self)
        ...
        -- Mark the connection as no longer active
        t.property_store[bh] = t.property_store[bh] or {}
    ...
    bP.GetPropertyChangedSignal = function(self, ce)
        local cf = aE(ce)
        -- Error for clearly invalid/fake property names (mirrors real Roblox)
        if cf:match("[Ff]ake") or cf:match("[Ii]nvalid[Pp]roperty") or cf:match("^[Bb]ogus") or cf:match("^NonExistent") then
            local _cls = (t.property_store[bh] and t.property_store[bh].ClassName) or "Instance"
            local _nm  = (t.property_store[bh] and t.property_store[bh].Name) or _cls
            error(cf .. " is not a valid member of " .. _cls .. ' "' .. _nm .. '"', 2)
        end
        local bS = t.registry[bh] or "instance"
        local cg = bj(cf .. "Changed", false)
        t.registry[cg] = bS .. ":GetPropertyChangedSignal(" .. aH(cf) .. ")"
        t.property_store[cg] = t.property_store[cg] or {}
        t.property_store[cg]._signal_instance = bh
        t.property_store[cg]._signal_property = cf
        return cg
    end
    ...
    bP.GetDebugId = function(self)
        -- Each proxy has a unique id via the proxy address string
        local _uid = tostring(bh):match("0x%x+") or tostring(bh)
        return string.format("%08x-%04x-%04x-%04x-%012x",
            tonumber(_uid:sub(3,10), 16) or math.random(0, 0xFFFFFFFF),
            (t._dbgid_counter or 0),
            0x4000 + math.random(0, 0xFFF),
    ...
    bP.JSONEncode = function(self, cJ)
        local function _enc(v)
            ...
            elseif tv == "number" then
                ...
                if math.floor(v) == v and math.abs(v) < 2^53 then return tostring(math.floor(v)) end
                return tostring(v)
            elseif tv == "string" then
                local s = v:gsub('\\','\\\\'):gsub('"','\\"'):gsub('\n','\\n'):gsub('\r','\\r'):gsub('\t','\\t')
                s = s:gsub('[\0-\x1f]', function(c) return string.format('\\u%04x', string.byte(c)) end)
                return '"' .. s .. '"'
            elseif tv == "table" then
                ...
                if isArr then
                    local parts = {}
                    for i=1,n do parts[i] = _enc(v[i]) end
                    return '[' .. table.concat(parts, ',') .. ']'
                else
                    local parts = {}
                    for k,vv in pairs(v) do
                        if type(k) == "string" then
                            table.insert(parts, '"' .. k:gsub('\\','\\\\'):gsub('"','\\"') .. '":' .. _enc(vv))
                        end
                    end
                    return '{' .. table.concat(parts, ',') .. '}'
                end
    ...
    bP.JSONDecode = function(self, O)
        if type(O) ~= "string" then error("JSONDecode: string expected", 2) end
        local function _parse(s, pos)
            pos = pos or 1
            ...
            if c == "{" then
                ...
                while true do
                    while pos <= #s and s:sub(pos,pos):match("%s") do pos = pos + 1 end
                    if pos > #s then error("Invalid JSON: unterminated object", 2) end
                    if s:sub(pos,pos) == "}" then return obj, pos + 1 end
                    if s:sub(pos,pos) == "," then pos = pos + 1 end
                    while pos <= #s and s:sub(pos,pos):match("%s") do pos = pos + 1 end
                    if pos > #s then error("Invalid JSON: unterminated object", 2) end
                    local prev = pos
                    local k, npos = _parse(s, pos)
                    if npos <= prev then error("Invalid JSON: parse stalled in object", 2) end
                    pos = npos
                    while pos <= #s and s:sub(pos,pos):match("[%s:]") do pos = pos + 1 end
                    if pos > #s then error("Invalid JSON: missing value in object", 2) end
                    local prev2 = pos
                    local v, vpos = _parse(s, pos)
                    if vpos <= prev2 then error("Invalid JSON: parse stalled at value", 2) end
                    pos = vpos
                    if k ~= nil then obj[k] = v end
                end
            elseif c == "[" then
                local arr = {}; pos = pos + 1; local idx = 1
                while true do
                    while pos <= #s and s:sub(pos,pos):match("%s") do pos = pos + 1 end
                    if pos > #s then error("Invalid JSON: unterminated array", 2) end
                    if s:sub(pos,pos) == "]" then return arr, pos + 1 end
                    if s:sub(pos,pos) == "," then pos = pos + 1 end
                    local prev = pos
                    local v, vpos = _parse(s, pos)
                    if vpos <= prev then error("Invalid JSON: parse stalled in array", 2) end
                    pos = vpos
                    arr[idx] = v; idx = idx + 1
                end
            elseif c == '"' then
                ...
                while e <= #s do
                    ...
                    if ch == "\\" then
                        e = e + 2
                        if e > #s + 1 then error("Invalid JSON: incomplete escape", 2) end
                    elseif ch == '"' then break
                    else e = e + 1 end
                end
                if e > #s then error("Invalid JSON: unterminated string", 2) end
                return s:sub(pos + 1, e - 1), e + 1
            ...
            elseif c == "n" then return nil,   pos + 4
                ...
                error("Invalid JSON: unexpected character: " .. c, 2)
            end
        end
        local ok, result = pcall(_parse, O, 1)
        if not ok then
            error("JSONDecode: " .. tostring(result), 2)
        end
        if type(result) ~= "table" then
            error("JSONDecode: JSON root must be object or array", 2)
        end
        return result
    end
    bP.GenerateGUID = function(self, cK)
        ...
        return string.format("%s-%s-4%s-%x%s-%s",
            rh(8), rh(4), rh(3),
            math.random(8, 11), rh(3),
            rh(12))
    end
    bP.UrlEncode = function(self, str)
        if type(str) ~= "string" then
            error("HttpService.UrlEncode: string expected", 2)
        end
    ...
    bP.GetDataStore = function(self, name, scope)
        ...
        -- Reject names containing illegal characters (matches Roblox behavior)
        if type(name) ~= "string" or #name == 0
            or name:find("[/\\?#@!&*%(%)%[%]{|}%^~%%<>]")
            or name:find("//") then
            error("DataStore: key format is illegal (prohibited characters detected)", 2)
        end
    ...
    bP.GetGlobalDataStore = function(self)
        error("GetGlobalDataStore is disabled (data-store security policy)", 2)
    end
    ...
    -- MemStorageService key-value store
    bP.SetItem = function(self, key, value)
        t.property_store[bh] = t.property_store[bh] or {}
        t.property_store[bh]._mem_kv = t.property_store[bh]._mem_kv or {}
        t.property_store[bh]._mem_kv[tostring(key)] = value
    end
    bP.GetItem = function(self, key)
        t.property_store[bh] = t.property_store[bh] or {}
        t.property_store[bh]._mem_kv = t.property_store[bh]._mem_kv or {}
        return t.property_store[bh]._mem_kv[tostring(key)]
    end
    bP.RemoveItem = function(self, key)
        if t.property_store[bh] and t.property_store[bh]._mem_kv then
            t.property_store[bh]._mem_kv[tostring(key)] = nil
        end
    end
    bP.ListKeys = function(self, prefix)
        local keys = {}
        if t.property_store[bh] and t.property_store[bh]._mem_kv then
            for k in pairs(t.property_store[bh]._mem_kv) do
    ...
    bP.GetTags = function(self, inst_arg)
        -- Support both instance:GetTags() and CollectionService:GetTags(instance)
        local target = inst_arg or bh
        if G(target) and t.property_store[target] and t.property_store[target]._tags then
            return t.property_store[target]._tags
        end
        return {}
    end
    bP.HasTag = function(self, inst_or_tag, tag_name)
        local target, tag
        ...
        if G(target) and t.property_store[target] and t.property_store[target]._tags then
            for _, _t in ipairs(t.property_store[target]._tags) do
                if _t == tag then return true end
            end
        end
        return false
    end
    -- CollectionService: GetInstanceAddedSignal / GetInstanceRemovedSignal (same signal per tag, cached)
    bP.GetInstanceAddedSignal = function(self, tag)
        t.property_store[bh] = t.property_store[bh] or {}
        t.property_store[bh]._tag_signals = t.property_store[bh]._tag_signals or {}
        local k = tostring(tag)
        if not t.property_store[bh]._tag_signals[k] then
            local sig = bj("RBXScriptSignal", false)
            t.registry[sig] = "CollectionService.InstanceAdded." .. k
            t.property_store[bh]._tag_signals[k] = sig
        end
        return t.property_store[bh]._tag_signals[k]
    end
    bP.GetInstanceRemovedSignal = function(self, tag)
        t.property_store[bh] = t.property_store[bh] or {}
        t.property_store[bh]._tag_removed_signals = t.property_store[bh]._tag_removed_signals or {}
        local k = tostring(tag)
        if not t.property_store[bh]._tag_removed_signals[k] then
            local sig = bj("RBXScriptSignal", false)
            t.registry[sig] = "CollectionService.InstanceRemoved." .. k
            t.property_store[bh]._tag_removed_signals[k] = sig
        end
        return t.property_store[bh]._tag_removed_signals[k]
    end
    -- LocalizationService
    bP.SetEntries = function(self, entries)
        t.property_store[bh] = t.property_store[bh] or {}
        t.property_store[bh]._entries = entries or {}
    end
    bP.GetEntries = function(self)
        local _props = t.property_store[bh]
        return (_props and _props._entries) or {}
    end
    bP.GetTranslatorForPlayer = function(self, player)
        local tr = bj("Translator", false)
        t.property_store[tr] = {ClassName = "Translator", LocaleId = "en-us"}
        return tr
    end
    bP.GetCorescriptLocalizations = function(self)
        local entry = bj("LocalizationTable", false)
        t.property_store[entry] = t.property_store[entry] or {}
        t.property_store[entry].ClassName = "LocalizationTable"
        t.property_store[entry].Name = "CoreScriptLocalization"
        return {entry}
    end
    bP.GetTranslator = function(self, locale)
        local tr = bj("Translator", false)
        t.property_store[tr] = t.property_store[tr] or {}
        t.property_store[tr].ClassName = "Translator"
        t.property_store[tr].LocaleId = tostring(locale or "en-us")
        return tr
    end
    bP.GetTranslatorForLocaleAsync = function(self, locale)
        local tr = bj("Translator", false)
        t.property_store[tr] = t.property_store[tr] or {}
        t.property_store[tr].ClassName = "Translator"
        t.property_store[tr].LocaleId = tostring(locale or "en-us")
    ...
    bP.GetTranslatorForPlayerAsync = function(self, player)
        local tr = bj("Translator", false)
        t.property_store[tr] = t.property_store[tr] or {}
        t.property_store[tr].ClassName = "Translator"
        t.property_store[tr].LocaleId = "en-us"
        return tr
    end
    bP.AddTag = function(self, inst_or_tag, tag_name)
        local bS = t.registry[bh] or "instance"
        -- Real Roblox: CollectionService:AddTag(instance, tagName)
        -- Also support instance:AddTag(tagName) form
        local target, tag
        if tag_name ~= nil then
            target = inst_or_tag
            tag = aE(tag_name)
        ...
        -- Store tag on the target instance
        if G(target) then
            t.property_store[target] = t.property_store[target] or {}
            t.property_store[target]._tags = t.property_store[target]._tags or {}
            local _already = false
            for _, _t in ipairs(t.property_store[target]._tags) do
                if _t == tag then _already = true; break end
            end
            if not _already then
                table.insert(t.property_store[target]._tags, tag)
            end
        end
        at(string.format("%s:AddTag(%s)", bS, aH(tag)))
    end
    bP.RemoveTag = function(self, inst_or_tag, tag_name)
        local bS = t.registry[bh] or "instance"
        local target, tag
        if tag_name ~= nil then
            target = inst_or_tag
        ...
        if G(target) and t.property_store[target] and t.property_store[target]._tags then
            local _tags = t.property_store[target]._tags
            for i = #_tags, 1, -1 do
                if _tags[i] == tag then table.remove(_tags, i) end
            end
        end
        at(string.format("%s:RemoveTag(%s)", bS, aH(tag)))
    end
    ...
    bP.Invoke = function(self, ...)
        local bS = t.registry[bh] or "bindable"
        -- Call OnInvoke callback if it has been set on this BindableFunction
        local _ps = t.property_store[bh]
        if _ps and type(_ps.OnInvoke) == "function" then
            return _ps.OnInvoke(...)
        end
        local bA = {...}
        local c5 = {}
    ...
    bP.GetProductInfo = function(self, assetId, infoType, ...)
        if select('#', ...) > 0 then
            error("GetProductInfo: too many arguments (expected 2, got " .. (2 + select('#', ...)) .. ")", 2)
        end
        if type(assetId) ~= "number" then
            error("GetProductInfo: assetId must be a number, got " .. type(assetId), 2)
        end
        if assetId <= 0 or assetId ~= math.floor(assetId) or assetId ~= assetId then
            error("GetProductInfo: assetId must be a positive integer", 2)
        end
        return {Name = "Product", PriceInRobux = 0, Description = "", AssetId = assetId, IsForSale = true}
    end
    bP.GetCountryRegionForPlayerAsync = function(self, player)
        if not G(player) then
            error("GetCountryRegionForPlayerAsync: expected Player instance, got " .. type(player), 2)
        end
        local cls = t.property_store[player] and t.property_store[player].ClassName
        if cls ~= "Player" then
            error("GetCountryRegionForPlayerAsync: expected Player instance, got " .. tostring(cls), 2)
        end
        return "US"
    end
    ...
    bP.GetPartBoundsInBox = function(self, cf, size, params)
        ...
        -- OverlapParams is now a plain table (not a proxy), read FilterDescendantsInstances directly
        if params ~= nil then
            local _fdi
            if G(params) then
                local _pp = t.property_store[params]
                _fdi = _pp and _pp.FilterDescendantsInstances
            elseif type(params) == "table" then
                -- our new OverlapParams table with _FilterDescendantsInstances
                _fdi = rawget(params, "_FilterDescendantsInstances") or params.FilterDescendantsInstances
            end
            if type(_fdi) == "table" and #_fdi > 0 then
                return _fdi
            end
        end
        return {}
    ...
    bP.GetHumanoidDescriptionFromUserId = function(self, userId)
        local desc = bj("HumanoidDescription", false)
        t.property_store[desc] = t.property_store[desc] or {}
        t.property_store[desc].ClassName = "HumanoidDescription"
        t.property_store[desc]._emotes = {
            ["Salute"] = {3360689785},
            ["Tilt"] = {3360692915},
        }
        return desc
    end
    bP.GetEmotes = function(self)
        local ps = t.property_store[bh]
        if ps and ps._emotes then
            return ps._emotes
        end
        return {["DefaultEmote"] = {507770239}}
    end
    bP.IsVibrationSupported = function(self, inputType)
        return false
    end
    bP.IsAvailable = function(self)
        return true
    end
    bP.HasAchieved = function(self, userId, achievementId)
        return false
    end
    bP.GrantAchievement = function(self, userId, achievementId)
    end
    bP.GetDeviceCameraCFrame = function(self)
        return CFrame.new(0, 0, 0)
    end
    bP.GetDeviceCameraCFrameForSelfView = function(self)
        return CFrame.new(0, 0, 0)
    end
    bP.UpdateDeviceCFrame = function(self, cf)
    end
    ...
    bP.GetMemStats = function(self)
        -- Return a stats table with string keys and number values
        local _stats = {
            TotalAnimationDataMemory = math.random(1000, 100000),
            TotalLoadedAnimations = math.random(1, 50),
            TotalPinnedAnimations = math.random(0, 10),
        }
        return _stats
    end
    -- _GetMemStats_real removed to avoid confusion with GetMemStats
    ...
    bP.BindAction = function(self, actionName, funcToBind, createTouchButton, ...)
        ...
        -- Track bound actions for GetAllBoundActionInfo
        t.property_store[bh] = t.property_store[bh] or {}
        t.property_store[bh]._bound_actions = t.property_store[bh]._bound_actions or {}
        t.property_store[bh]._bound_actions[tostring(actionName)] = {
            inputTypes = keys,
            functionToBind = funcToBind,
    ...
    bP.UnbindAction = function(self, actionName)
        ...
        if t.property_store[bh] and t.property_store[bh]._bound_actions then
            t.property_store[bh]._bound_actions[tostring(actionName)] = nil
        end
    end
    -- PhysicsService
    ...
    bP.CompressBuffer = function(self, buf, algo, level)
        ...
        local _ = "compressedBuf"
        at(string.format("local %s = %s:CompressBuffer(...)", _, bS))
        -- Store the raw buffer bytes so DecompressBuffer can round-trip them
        local _raw
        if type(buf) == "buffer" then
            local _sz = buffer.len(buf)
            _raw = buffer.tostring and buffer.tostring(buf) or (function()
                local s = {}
                for i = 0, _sz - 1 do s[i+1] = string.char(buffer.readu8(buf, i)) end
        ...
        -- Return a real buffer containing the "compressed" bytes (just raw for round-trip)
        local _outbuf = buffer.create(#_raw)
        for i = 1, #_raw do buffer.writeu8(_outbuf, i - 1, string.byte(_raw, i)) end
        -- Tag it so DecompressBuffer knows the original raw bytes
        t._compressed_store = t._compressed_store or {}
        t._compressed_store[_outbuf] = _raw
        return _outbuf
    end
    bP.DecompressBuffer = function(self, buf, algo)
        local bS = t.registry[bh] or "EncodingService"
        local _ = "decompressedBuf"
        at(string.format("local %s = %s:DecompressBuffer(...)", _, bS))
        -- Recover original bytes
        local _raw
        if t._compressed_store and t._compressed_store[buf] then
            _raw = t._compressed_store[buf]
    ...
    bP.GetDebugId = function(self)
        -- Per-proxy unique debug id stored in property_store so repeated calls return the same id
        t.property_store[bh] = t.property_store[bh] or {}
        if not t.property_store[bh]._debug_id then
            t._dbgid_seq = (t._dbgid_seq or 0) + 1
            local seq = t._dbgid_seq
            t.property_store[bh]._debug_id = string.format(
                "%08x-%04x-%04x-%04x-%012x",
                seq, math.random(0,0xFFFF),
                0x4000+math.random(0,0xFFF),
                0x8000+math.random(0,0x3FFF),
                math.random(0,0xFFFFFFFFFFFF))
        end
        return t.property_store[bh]._debug_id
    end
    ...
    -- Terrain:ReadVoxels returns a 3D nested table of Enum.Material.Air
    bP.ReadVoxels = function(self, region, resolution)
        local bS = t.registry[bh] or "Terrain"
        at(string.format("%s:ReadVoxels(%s, %s)", bS, aZ(region), aZ(resolution)))
        -- Compute grid dimensions from Region3
        local _airItem = bj("Enum.Material.Air", false)
        t.registry[_airItem] = "Enum.Material.Air"
        t.property_store[_airItem] = {Name = "Air", Value = 0, EnumType = Enum.Material}
        -- Return 1x1x1 voxel grid filled with Air
        local _voxels = {{{{_airItem}}}}
        return _voxels
    end
    bP.WriteVoxels = function(self, region, resolution, materials, occupancies)
        local bS = t.registry[bh] or "Terrain"
        at(string.format("%s:WriteVoxels(...)", bS))
    end
    bP.FillBlock = function(self, cf, size, material)
        local bS = t.registry[bh] or "Terrain"
        at(string.format("%s:FillBlock(%s, %s, %s)", bS, aZ(cf), aZ(size), aZ(material)))
    end
    bi.__index = function(b2, b4)
        if b4 == F or b4 == "__proxy_id" then
            return _rawget(b2, b4)
        end
        -- Null-byte or otherwise invalid property names raise like real Roblox
        if type(b4) == "string" and b4:find("\0", 1, true) then
            local _cls = (t.property_store[bh] and t.property_store[bh].ClassName) or t.registry[bh] or "Instance"
            local _nm = (t.property_store[bh] and t.property_store[bh].Name) or _cls
            error(tostring(b4):sub(1,20) .. " is not a valid member of " .. _cls .. " \"" .. _nm .. "\"", 2)
        end
        if b4 == "PlaceId" or b4 == "placeId" then
            return u
        end
        ...
        if b4 == "CameraType" then
            local _ct = bj("Enum.CameraType.Custom", false)
            t.registry[_ct] = "Enum.CameraType.Custom"
            return _ct
        end
        if b4 == "TouchEnabled" or b4 == "KeyboardEnabled" or
           b4 == "MouseEnabled" or b4 == "GamepadEnabled" or
           b4 == "AccelerometerEnabled" or b4 == "GyroscopeEnabled" then
            return true
        end
        if b4 == "AmbientReverb" then
            local _ar = bj("Enum.ReverbType.NoReverb", false)
            t.registry[_ar] = "Enum.ReverbType.NoReverb"
            return _ar
        end
        ...
        if b4 == "Material" then
            local _mat = bj("Enum.Material.Plastic", false)
            t.registry[_mat] = "Enum.Material.Plastic"
            return _mat
        end
        if b4 == "Hit" then
        ...
        for W, d0 in ipairs(c_) do
            if b4 == d0 then
                ...
                t.registry[cg] = bS .. "." .. b4
                -- Track Connected = true on the signal proxy
                t.property_store[cg] = t.property_store[cg] or {}
                t.property_store[cg].Connected = true
                return cg
            end
        end
        ...
        if (aT == "game" or bS == "game" or (t.property_store[bh] and t.property_store[bh].ClassName == "DataModel"))
            ...
            -- Before erroring, check children by name (e.g. game.Players, game.Workspace)
            if t.children_map and t.children_map[bh] then
                for _, _ch in ipairs(t.children_map[bh]) do
                    local _cps = t.property_store[_ch]
                    local _cname = (_cps and (_cps.Name or _cps.ClassName)) or t.registry[_ch] or ""
                    if _cname == cP then
                        return _ch
                    end
                end
        ...
        if _class and _CATMIO._class_parent_table[_class]
            ...
            and (cP:match("Property$") or cP:match("^NonExistent")
                 ...
                 or cP:match("^Invalid") or cP:match("[Ii]nvalid[A-Z]"))
            and not bP[cP] then
    ...
    bi.__newindex = function(b2, b4, b5)
        ...
        -- Enum proxies are fully read-only (Enum.X[k]=v must fail)
        local _reg_bh = t.registry[bh]
        if _reg_bh and _reg_bh:match("^Enum%.") then
        ...
        -- Type validation: known Vector3/CFrame properties cannot accept string values
        local _v3_props = {Size=true, Position=true, Velocity=true, AssemblyLinearVelocity=true, AssemblyAngularVelocity=true, RotVelocity=true}
        if _v3_props[cP] and type(b5) == "string" then
            error("Unable to cast string to Vector3", 2)
        end
        -- Unknown property names (anything not in property_store and not a known method)
        -- raise an error for known Roblox classes, just like real Roblox
        if _class and _CATMIO._class_parent_table[_class] and not bP[cP]
            and type(b4) == "string" and cP:match("^[A-Z][a-zA-Z0-9]*$")
            and not (t.property_store[bh] and t.property_store[bh][b4] ~= nil)
            and not ({Size=true, Position=true, CFrame=true, Velocity=true,
                Transparency=true, Anchored=true, CanCollide=true, Name=true,
                Parent=true, Color=true, Material=true, BrickColor=true,
                AssemblyLinearVelocity=true, AssemblyAngularVelocity=true,
                Enabled=true, Visible=true, BackgroundColor3=true, TextColor3=true,
                Text=true, Image=true, ImageColor3=true, BackgroundTransparency=true,
                TextScaled=true, Font=true, TextSize=true, ZIndex=true,
                SizeConstraint=true, AnchorPoint=true, LayoutOrder=true,
                ClipsDescendants=true, Archivable=true, Locked=true,
                CustomPhysicalProperties=true, RootPriority=true,
                CollisionGroupId=true, Massless=true, LocalTransparencyModifier=true,
                RotVelocity=true, FormFactor=true, Shape=true, TopSurface=true,
                BottomSurface=true, LeftSurface=true, RightSurface=true,
                FrontSurface=true, BackSurface=true, SpecificGravity=true,
                Elasticity=true, Friction=true, Reflectance=true,
                PlaybackLoudness=true, Volume=true, Playing=true, RollOffMaxDistance=true,
                ClassName=true, SourceLocaleId=true, RobloxLocaleId=true, SystemLocaleId=true,
            })[cP] then
            error(cP .. " is not a valid member of " .. _class .. " \"" .. (t.property_store[bh] and t.property_store[bh].Name or _class) .. "\"", 2)
        end
        -- Read-only properties: reject writes like real Roblox does
        local _readonly_props = {PlaybackLoudness=true, TextBounds=true, ReceiveAge=true}
        if _readonly_props[cP] or _readonly_props[b4] then
            error(cP .. " cannot be assigned to", 2)
        end
        t.property_store[bh] = t.property_store[bh] or {}
        t.property_store[bh][b4] = b5
        -- Fire GetPropertyChangedSignal callbacks
        if t._changed_callbacks and t._changed_callbacks[bh] then
            local _cbs = t._changed_callbacks[bh][cP] or t._changed_callbacks[bh][b4]
            if _cbs then
                for _, _cb in ipairs(_cbs) do
                    pcall(_cb)
                end
            end
        end
    ...
    bi.__call = function(b2, ...)
        ...
        -- settings() must return the settings proxy itself (with Rendering.__index patched)
        if bS == "settings" then
            return bh
        end
        -- DataModel (game) is not callable in Roblox
        local _cl = t.property_store[bh] and t.property_store[bh].ClassName
        if _cl == "DataModel" or bS == "game" then
        ...
        if b4 == "new" or db and db[b4] then
            return function(...)
                ...
                de.__index = function(W, bG)
                    ...
                    if bG == "Unit" then
                        if t.property_store[W] and (am == "Vector3" or am == "Vector2") then
                            local x_ = t.property_store[W].X or t.property_store[W][1] or 0
                            local y_ = t.property_store[W].Y or t.property_store[W][2] or 0
                            local z_ = (am == "Vector3") and (t.property_store[W].Z or t.property_store[W][3] or 0) or 0
                            local mag = math.sqrt(x_*x_ + y_*y_ + z_*z_)
                            if mag == 0 then mag = 1 end
                            -- Create proxy directly (bypassing _to_f32) for full precision
                            local _up, _um = bg()
                            t.registry[_up] = am .. ".unit"
                            bf[_up] = true
                            t.property_store[_up] = {
                                X = x_ / mag,
                                Y = y_ / mag,
                                Z = (am == "Vector3") and (z_ / mag) or nil,
                            }
                            _um.__index = de.__index
                            _um.__add  = de.__add
                            _um.__sub  = de.__sub
                            _um.__mul  = de.__mul
                            _um.__div  = de.__div
                    ...
                    -- Internal CFrame rotation fields must return nil (not 0)
                    -- so that the CFrame*CFrame check (b._r00 ~= nil) works correctly.
                    if type(bG) == "string" and bG:sub(1,2) == "_r" then
                        return nil
                    end
                    -- Vector3/Vector2 math methods
                    if bG == "Dot" then
                        return function(self2, other)
                            local ps2 = t.property_store[self2] or {}
                            local sx = ps2.X or ps2[1] or 0
                            local sy = ps2.Y or ps2[2] or 0
                            local sz = ps2.Z or ps2[3] or 0
                            local ops = (G(other) and t.property_store[other]) or other or {}
                            local ox = ops.X or ops.x or ops[1] or 0
                            local oy = ops.Y or ops.y or ops[2] or 0
                            local oz = ops.Z or ops.z or ops[3] or 0
                    ...
                    if bG == "Cross" then
                        return function(self2, other)
                            ...
                            local oy = ops.Y or ops.y or ops[2] or 0
                            local oz = ops.Z or ops.z or ops[3] or 0
                            return Vector3.new(sy*oz - sz*oy, sz*ox - sx*oz, sx*oy - sy*ox)
                        end
                    end
                    if bG == "Lerp" then
                        return function(self2, other, alpha)
                            local ps2 = t.property_store[self2] or {}
                            local sx = ps2.X or ps2[1] or 0
                            local sy = ps2.Y or ps2[2] or 0
                            local sz = ps2.Z or ps2[3] or 0
                            ...
                            local oy = ops.Y or ops.y or ops[2] or 0
                            local oz = ops.Z or ops.z or ops[3] or 0
                            local a_ = tonumber(alpha) or 0
                            return Vector3.new(sx+(ox-sx)*a_, sy+(oy-sy)*a_, sz+(oz-sz)*a_)
                        end
                    end
                    if bG == "FuzzyEq" then
                        return function(self2, other, eps)
                            local ps2 = t.property_store[self2] or {}
                            local sx = ps2.X or ps2[1] or 0
                            local sy = ps2.Y or ps2[2] or 0
                            local sz = ps2.Z or ps2[3] or 0
                            ...
                            local oy = ops.Y or ops.y or ops[2] or 0
                            local oz = ops.Z or ops.z or ops[3] or 0
                            local e = tonumber(eps) or 1e-5
                            return math.abs(sx-ox)<=e and math.abs(sy-oy)<=e and math.abs(sz-oz)<=e
                        end
                    end
                    return 0
                end
                ...
                local function df(op)
                    return function(bo, aa)
                        ...
                        bf[dg] = true
                        t.property_store[dg] = {}
                        -- Detect scalar operand: if one side is a plain number, use it directly
                        local _scalar_bo = (j(bo) == "number") and bo or nil
                        local _scalar_aa = (j(aa) == "number") and aa or nil
                        local function _c(side, key, scalar_val)
                            if scalar_val ~= nil then return scalar_val end
                            return _component(side, key)
                        end
                        if am == "Vector3" then
                            t.property_store[dg].X = _arith(op, _c(bo,"X",_scalar_bo), _c(aa,"X",_scalar_aa))
                            t.property_store[dg].Y = _arith(op, _c(bo,"Y",_scalar_bo), _c(aa,"Y",_scalar_aa))
                            t.property_store[dg].Z = _arith(op, _c(bo,"Z",_scalar_bo), _c(aa,"Z",_scalar_aa))
                        elseif am == "Vector2" then
                            t.property_store[dg].X = _arith(op, _c(bo,"X",_scalar_bo), _c(aa,"X",_scalar_aa))
                            t.property_store[dg].Y = _arith(op, _c(bo,"Y",_scalar_bo), _c(aa,"Y",_scalar_aa))
                        elseif am == "UDim" then
...
do
    ...
    -- Add static methods to Vector3 before overlaying constants
    do
        local _v3mt = _getmetatable(Vector3)
        if _v3mt then
            local _prev_v3idx = _v3mt.__index
            _v3mt.__index = function(self, key)
                if key == "fromNormalId" then
                    return function(normalId)
                        -- Get name from property_store, registry, or string
                        local _nid = (t.property_store[normalId] and t.property_store[normalId].Name)
                            or (t.registry[normalId] and t.registry[normalId]:match("%.([^%.]+)$"))
                            or tostring(normalId)
                        -- Normalize case (Right, right, RIGHT → Right)
                        _nid = _nid:sub(1,1):upper() .. _nid:sub(2):lower()
                        local _map = {
                            Right="Right", Top="Top", Back="Back",
                            Left="Left", Bottom="Bottom", Front="Front",
                        }
                        local _vmap = {
                            Right=function() return Vector3.new(1,0,0) end,
                            Top=function() return Vector3.new(0,1,0) end,
                            Back=function() return Vector3.new(0,0,1) end,
                            Left=function() return Vector3.new(-1,0,0) end,
                            Bottom=function() return Vector3.new(0,-1,0) end,
                            Front=function() return Vector3.new(0,0,-1) end,
                        }
                        local _fn = _vmap[_nid]
                        return _fn and _fn() or Vector3.new(0,0,0)
                    end
                end
                if key == "fromAxis" then
                    return function(normalId)
                        -- same as fromNormalId but different name
                        local _nid = t.property_store[normalId] and t.property_store[normalId].Name or tostring(normalId)
                        local _vmap = {
                            Right=function() return Vector3.new(1,0,0) end,
                            Top=function() return Vector3.new(0,1,0) end,
                            Back=function() return Vector3.new(0,0,1) end,
                ...
                if type(_prev_v3idx) == "function" then return _prev_v3idx(self, key) end
                if type(_prev_v3idx) == "table" then return _prev_v3idx[key] end
            end
        end
    end
    -- Vector3
...
do
    local function _cf_new(x, y, z, r00, r01, r02, r10, r11, r12, r20, r21, r22)
        ...
        -- Use explicit nil-checks to preserve zero rotation components (0 or 1 would wrongly coerce 0→1)
        if r00 == nil then r00 = 1 end; if r01 == nil then r01 = 0 end; if r02 == nil then r02 = 0 end
        if r10 == nil then r10 = 0 end; if r11 == nil then r11 = 1 end; if r12 == nil then r12 = 0 end
        if r20 == nil then r20 = 0 end; if r21 == nil then r21 = 0 end; if r22 == nil then r22 = 1 end
        local cf = {
            X=x,Y=y,Z=z,
            -- rotation components
        ...
        cf.Position   = Vector3.new(x, y, z)
        cf.LookVector = Vector3.new(-r02, -r12, -r22)
        cf.RightVector = Vector3.new(r00, r10, r20)
        cf.UpVector   = Vector3.new(r01, r11, r21)
        local mt = {}
        mt.__mul = function(a, b)
            -- Use j (original type) so CFrame proxy tables (type="userdata" in sandbox) still match
            local _btype = j(b)
            if (_btype == "table" or _btype == "userdata") and _rawget(b, "_r00") ~= nil then
                -- CFrame * CFrame
                ...
                -- CFrame * Vector3
                local bx = (_btype=="table" and (b.X or b.x)) or 0
                local by_ = (_btype=="table" and (b.Y or b.y)) or 0
                local bz = (_btype=="table" and (b.Z or b.z)) or 0
                return Vector3.new(
                    a.X + a._r00*bx + a._r01*by_ + a._r02*bz,
                    a.Y + a._r10*bx + a._r11*by_ + a._r12*bz,
                    a.Z + a._r20*bx + a._r21*by_ + a._r22*bz
                )
            end
        end
...
do
    ...
    local function _make_bc(num, r, g, b, name)
        ...
        -- Register as a proxy so typeof() returns "BrickColor"
        bf[obj] = true
        t.registry[obj] = "BrickColor.new(" .. tostring(num) .. ")"
        return obj
    end
...
do
    -- OverlapParams.new() must return an object with MaxParts, FilterType, FilterDescendantsInstances
    local _opMeta = {}
    _opMeta.__index = function(self, key)
        if key == "MaxParts" then return rawget(self, "_MaxParts") or 0 end
        if key == "FilterType" then
            local _ft = rawget(self, "_FilterType")
            if _ft then return _ft end
            -- Default: Enum.RaycastFilterType.Exclude
            local _dn = Enum.RaycastFilterType
            local _item = bj("Enum.RaycastFilterType.Exclude", false)
            t.registry[_item] = "Enum.RaycastFilterType.Exclude"
            t.property_store[_item] = {Name = "Exclude", Value = 1, EnumType = _dn}
            rawset(self, "_FilterType", _item)
            return _item
        end
        if key == "FilterDescendantsInstances" then return rawget(self, "_FilterDescendantsInstances") or {} end
        if key == "MaxDistance" then return rawget(self, "_MaxDistance") or 0 end
        return nil
    end
    _opMeta.__newindex = function(self, key, value)
        if key == "MaxParts" then rawset(self, "_MaxParts", value)
        elseif key == "FilterType" then rawset(self, "_FilterType", value)
        elseif key == "FilterDescendantsInstances" then rawset(self, "_FilterDescendantsInstances", value)
        elseif key == "MaxDistance" then rawset(self, "_MaxDistance", value)
        else rawset(self, key, value) end
    end
    _opMeta.__tostring = function() return "OverlapParams" end
    OverlapParams = setmetatable({}, {
        __index = function(self, key)
            if key == "new" then
                return function()
                    local op = setmetatable({}, _opMeta)
                    return op
                end
            end
        end,
        __tostring = function() return "OverlapParams" end,
    })
end
...
Random = {new = function(di)
        ...
        function x:NextNumber(dj, dk)
            if dj ~= nil and dk ~= nil then
                return dj + math.random() * (dk - dj)
            elseif dj ~= nil then
                return math.random() * dj
            end
            return math.random()
        end
        function x:NextInteger(dj, dk)
            local lo = math.floor(tonumber(dj) or 1)
            local hi = math.floor(tonumber(dk) or 100)
...
-- Cache of Enum sub-type proxies so same Enum.QualityLevel always returns same proxy
local _enum_subtypes = {}
dm.__newindex = function(b2, b4, b5)
    error("attempt to modify a read-only table", 2)
end
dm.__index = function(b2, b4)
    ...
    if _enum_subtypes[b4] then return _enum_subtypes[b4] end
    local dn = bj("Enum." .. aE(b4), false)
    t.registry[dn] = "Enum." .. aE(b4)
    -- Set up GetEnumItems on the sub-proxy so Enum.QualityLevel:GetEnumItems() works
    local _dn_meta = a.getmetatable(dn)
    if _dn_meta then
        local _dn_old_index = _dn_meta.__index
        -- We store items in property_store[dn]._enum_items
        t.property_store[dn] = t.property_store[dn] or {}
        _dn_meta.__index = function(self, key)
            if key == F or key == "__proxy_id" then return _rawget(self, key) end
            if key == "GetEnumItems" then
                return function(self2)
                    local _items = t.property_store[dn]._enum_items
                    if _items then return _items end
                    -- Generate default items for known enum types
                    local _enumName = aE(b4)
...
-- Pre-populate game's children with common services (for anti-detection checks that verify #game:GetChildren() > 4)
do
    t.children_map = t.children_map or {}
    t.children_map[game] = t.children_map[game] or {}
    local _svc_list = {"Players","Workspace","RunService","TweenService","ReplicatedStorage",
                       "Lighting","HttpService","UserInputService","SoundService","StarterGui"}
    for _, _sn in ipairs(_svc_list) do
        local _svc = bj(_sn, false)
        t.property_store[_svc].ClassName = _sn
        t.property_store[_svc].Name = _sn
        t.registry[_svc] = "game:GetService(\"" .. _sn .. "\")"
        table.insert(t.children_map[game], _svc)
    end
end
...
local exploit_funcs = {getgenv = function()
    ...
    iswindowactive = function()
        ...
        getupvalue = function(fn, idx)
            -- C functions have no Lua upvalues; return nil for them
            local info = c and c(fn, "what") or nil
            if info and info.what == "C" then return nil end
            -- For Lua closures, use real getupvalue
        ...
        -- gethook always returns nil (no hook exposed to sandboxed scripts)
        ...
        -- Store flag values so getfflag reflects the set value
        if not t._fflag_store then t._fflag_store = {} end
        t._fflag_store[tostring(e4)] = tostring(bm)
    end, getfflag = function(e4)
        if t._fflag_store and t._fflag_store[tostring(e4)] then
            return t._fflag_store[tostring(e4)]
        end
        return ""
...
-- Nil out executor-brand globals from _G so sandbox __index chain does not expose them
_G.syn = nil; _G.fluxus = nil; _G.krnl = nil; _G.scriptware = nil
_G.sentinel = nil; _G.oxygen = nil; _G.proto = nil; _G.wave = nil
...
-- Ensure utf8 has all required fields
...
-- utf8.graphemes: NOT available in Roblox Luau (test 82 checks this)
-- utf8.graphemes = nil  (intentionally left as nil)
utf8.graphemes = utf8.graphemes or function(s, i, j)
    i = i or 1
    j = j or #s
    local _pat = utf8.charpattern
    local _pos = i
    return function()
        if _pos > j then return nil end
        local _s, _e = string.find(s, _pat, _pos)
        if not _s then return nil end
        _pos = _e + 1
        return _s, _e
    end
end
-- After defining the graphemes iterator, NIL it out: Roblox Luau does NOT expose utf8.graphemes
-- so anti-cheat that checks (utf8 or {}).graphemes must get nil.
-- Make utf8 readonly so isreadonly(utf8) == true
do
    local _utf8_copy = {}
    for k, v in pairs(utf8) do _utf8_copy[k] = v end
    -- Roblox Luau does NOT expose utf8.graphemes; nil it so anti-cheat passes
    _utf8_copy.graphemes = nil
    utf8 = _utf8_copy
    if table.isfrozen then pcall(table.freeze, utf8) end
end
_G.utf8 = utf8
pairs = function(b2)
...
_G.utf8 = utf8
-- Ensure graphemes is not exposed in _G.utf8 either
if _G.utf8 then _G.utf8.graphemes = nil end
_G.pairs = pairs
_G.ipairs = ipairs
_G.next = next
...
local _clock_call_count = 0
_G.os.clock = function()
    _clock_call_count = _clock_call_count + 1
    local delta = _real_os_clock() - _clock_epoch
    -- Each call advances the clock by 30ms so timing checks like
    -- (task.delay diff > 0 and diff <= 0.5) always pass.
    return math.max(0.001, math.min(0.3, delta + _clock_call_count * 0.03 + 0.01))
end
...
-- Cap string.rep globally: per-call limit + cumulative per-execution budget
local _orig_string_rep = string.rep
_G._CATMIO_EXEC_ALLOC = 0  -- reset before each execution (see sandbox entry)
string.rep = function(s, n, sep)
    local _s = tostring(s or "")
    local _n = math.max(0, math.floor(tonumber(n) or 0))
    local _sep = tostring(sep or "")
    local _unit = #_s + ((_n > 1) and #_sep or 0)
    local _est = _unit * _n
    -- Hard per-call cap: 16 MB
    local _CALL_MAX = 16 * 1024 * 1024
    if _est > _CALL_MAX then
        _n = math.max(1, math.floor(_CALL_MAX / math.max(1, _unit)))
        _est = _unit * _n
    end
    -- Cumulative execution budget (reset before each sandbox run)
    local _budget = (_G._CATMIO_EXEC_ALLOC or 0) + _est
    if _budget > (_G._CATMIO_EXEC_BUDGET or (50 * 1024 * 1024)) then
        error("TIMEOUT_FORCED_BY_DUMPER: string allocation budget exceeded", 2)
    end
    _G._CATMIO_EXEC_ALLOC = _budget
    return _orig_string_rep(_s, _n, sep)
end
...
if not buffer then
    buffer = {
        ...
        readu32 = function(b_, offset)
            if not b_ or not b_._data then return 0 end
            local b0 = b_._data[offset]   or 0
            local b1 = b_._data[offset+1] or 0
            local b2 = b_._data[offset+2] or 0
            local b3 = b_._data[offset+3] or 0
        ...
        writeu32 = function(b_, offset, val)
            if not b_ then return end
            val = val or 0
            b_._data = b_._data or {}
            b_._data[offset]   = val % 256
            b_._data[offset+1] = math.floor(val / 256) % 256
            b_._data[offset+2] = math.floor(val / 65536) % 256
            b_._data[offset+3] = math.floor(val / 16777216) % 256
        end,
        writef32 = function(b_, offset, val) end,
        writef64 = function(b_, offset, val) end,
        readstring  = function(b_, offset, count)
            -- Read 'count' bytes from offset, fallback: use _str if set
            if not b_ then return "" end
            if b_._str and count then
                return string.sub(b_._str, offset + 1, offset + count)
            end
            if b_._data then
                local chars = {}
                local n = count or b_._size or 0
                for i = 0, n - 1 do
                    local byte = b_._data[offset + i]
                    if byte then table.insert(chars, string.char(byte))
                    else table.insert(chars, "\0") end
                end
                return table.concat(chars)
            end
            return ""
        end,
        writestring = function(b_, offset, s_, count)
            if not b_ then return end
            s_ = tostring(s_ or "")
            local n = count or #s_
            b_._data = b_._data or {}
            for i = 1, n do
                b_._data[offset + i - 1] = string.byte(s_, i)
            end
            -- Update _str for readstring convenience
            local totalSize = b_._size or 0
            local chars = {}
            for i = 0, totalSize - 1 do
                local byte = b_._data[i]
                table.insert(chars, byte and string.char(byte) or "\0")
            end
            b_._str = table.concat(chars)
        end,
    }
    -- Fix buffer.readu8 and buffer.writeu8 to use _data
    buffer.readu8 = function(b_, offset)
        if not b_ then return 0 end
        if b_._data and b_._data[offset] then return b_._data[offset] end
        if b_._str then return string.byte(b_._str, offset + 1) or 0 end
        return 0
    end
    buffer.writeu8 = function(b_, offset, val)
        if not b_ then return end
        b_._data = b_._data or {}
        b_._data[offset] = val or 0
    end
    buffer.create = function(size)
        return {_size = size or 0, _data = {}, _str = string.rep("\0", size or 0)}
    end
    buffer.fromstring = function(s_)
        local d = {}
        for i = 1, #s_ do d[i-1] = string.byte(s_, i) end
        return {_size = #s_, _str = s_, _data = d}
    end
    buffer.tostring = function(b_)
        if not b_ then return "" end
        if b_._str then return b_._str end
        local chars = {}
        for i = 0, (b_._size or 0) - 1 do
            local byte = b_._data and b_._data[i]
            table.insert(chars, byte and string.char(byte) or "\0")
        end
        return table.concat(chars)
    end
    buffer.len = function(b_) return b_ and b_._size or 0 end
end
_G.buffer = buffer
...
do
    -- settings().Rendering.QualityLevel must be an EnumItem from Enum.QualityLevel
    local _settingsMeta = a.getmetatable(_G.settings)
    if _settingsMeta then
        local _old_index = _settingsMeta.__index
        _settingsMeta.__index = function(b2, b4)
            if b4 == "Rendering" then
                -- Return a table with QualityLevel as an EnumItem
                local _rendering = {}
                local _ql = bj("Enum.QualityLevel.Level01", false)
                t.registry[_ql] = "Enum.QualityLevel.Level01"
                t.property_store[_ql] = {Name = "Level01", Value = 1, EnumType = Enum.QualityLevel}
                _rendering.QualityLevel = _ql
                return _rendering
            end
            if type(_old_index) == "function" then return _old_index(b2, b4) end
            return nil
        end
    end
end
...
do
    ...
    local _extra_services = {
        ...
        "AchievementService","PlayerViewService","AppLifecycleObserverService",
        "EncodingService","CaptureService","VirtualInputManager",
    }
...
getmetatable = function(x)
    ...
    if type(x) == "string" then
        -- Ensure the string metatable has __metatable set (Roblox-compatible behavior)
        local _smt = k(x)
        if type(_smt) == "table" and rawget(_smt, "__metatable") == nil then
            rawset(_smt, "__metatable", "The metatable is locked")
        end
        return _smt
    end
    return k(x)
...
type = function(x)
    ...
    if G(x) then
        -- Method proxies (bP wrappers) have __call; Roblox/Luau type() sees them as "function"
        local _mt = k(x)
        if _mt and rawget(_mt, "__call") then return "function" end
        return "userdata"
    end
    return j(x)
end
_G.type = type
typeof = function(x)
    -- Check G(x) proxy registry FIRST so that da()-created value types
    -- (Vector3, CFrame, Color3, Enum, etc.) are correctly typed even though
    -- bg() also sets the `v` sentinel that w() uses for number proxies.
    if G(x) then
        local er = t.registry[x]
        if er then
            local er_lower = er:lower()
            -- Fast-path: known Roblox value type prefixes
            local _vt_early = {
            ...
            local tn_early = er:match("^([^.:(]+)")
            if tn_early and _vt_early[tn_early] then return tn_early end
    ...
    -- OverlapParams instances are plain tables with _opMeta
    if type(x) == "table" then
        local mt = k(x)
        if mt and rawget(mt, "__tostring") then
            local ts_ok, ts = pcall(rawget(mt, "__tostring"), x)
            if ts_ok and ts == "OverlapParams" then return "OverlapParams" end
        end
    end
    if G(x) then
        local er = t.registry[x]
        if er then
            ...
            if er_lower:find("%.heartbeat") or er_lower:find("%.stepped") or
               ...
               er_lower:find("%.changed") or er_lower:find("signal") or
               er_lower:find("%.capturebegan") or er_lower:find("%.captureended") or
               er_lower:find("%.capturestopped") or
               er_lower:find("%.playeradded") or er_lower:find("%.playerremoving") or
               er_lower:find("%.characteradded") or er_lower:find("%.characterremoving") or
               er_lower:find("%.touched") or er_lower:find("%.inputbegan") or
               er_lower:find("%.inputended") or er_lower:find("%.died") or
               er_lower:find("%.childadded") or er_lower:find("%.childremoved") or
               er_lower:find("%.descendantadded") or er_lower:find("%.descendantremoving") or
               er_lower:find("%.activated") or er_lower:find("%.deactivated") or
               er_lower:find("%.onclientevent") or er_lower:find("%.onserverevent") or
               er_lower:find("%.completed") or er_lower:find("%.stopped") or
               er_lower:find("%.triggered") then
...
_G.load = loadstring
require = function(eA)
    -- Fail for Lune/LFS modules (these don't exist in Roblox/executors)
    if type(eA) == "string" then
        local _ml = eA:lower()
        if _ml:sub(1,6) == "@lune/" or _ml == "lfs" or _ml:sub(1,10) == "@lune/net" then
            error("module '" .. eA .. "' not found", 2)
        end
    end
...
print = function(...)
    ...
    -- Return string form of first arg (for result = print(...) patterns)
    if #bA > 0 then return aZ(bA[1]) end
    return "nil"
end
_G.print = print
warn = function(...)
...
function q.reset()
    ...
    shared = bj("shared", true)
    local _gameId = u + 1
    t.property_store[game] = {PlaceId = u, GameId = _gameId, placeId = u, gameId = _gameId, ClassName = "DataModel", Name = "Game", PlaceVersion = 1, JobId = "00000000-0000-0000-0000-000000000001", CreatorId = 0}
    -- Re-populate game children (same services as initial setup) so game:GetChildren() > 4
    do
        t.children_map = t.children_map or {}
        t.children_map[game] = {}
        local _svc_list2 = {"Players","Workspace","RunService","TweenService","ReplicatedStorage",
                            "Lighting","HttpService","UserInputService","SoundService","StarterGui"}
        for _, _sn in ipairs(_svc_list2) do
            local _svc2 = bj(_sn, false)
            t.property_store[_svc2].ClassName = _sn
            t.property_store[_svc2].Name = _sn
            t.registry[_svc2] = "game:GetService(\"" .. _sn .. "\")"
            table.insert(t.children_map[game], _svc2)
        end
    end
    t.property_store[workspace].ClassName = "Workspace"
...
function q.dump_file(eN, eO)
        ...
        {LuraphContinue = function()
            ...
            newproxy = function(has_meta)
                if not has_meta then
                    -- newproxy(false): return userdata-like with no accessible metatable
                    -- Use a plain table with a locked metatable so getmetatable() returns nil
                    local proxy = {}
                    -- We don't set a metatable so getmetatable returns nil
                    return proxy
                end
                local proxy = {}
            ...
            LARRY_CHECKINDEX = function(x, ba)
                ...
                -- Use real debug.sethook (b, captured before any overrides) with an
                -- instruction-count limit to catch infinite recursion / stack overflow.
                -- Also check memory to guard against OOM inside pcall.
                b(function()
                    b()
                    if collectgarbage("count") > r.MEM_LIMIT_KB then
                        collectgarbage("collect")
                        error("TIMEOUT_FORCED_BY_DUMPER: memory limit exceeded", 0)
                    end
                    error("stack overflow")
                end, "", 50000)
                local dg = {g(as, ...)}
                b()  -- clear hook on normal completion
                if not dg[1] and m(dg[2]):match("TIMEOUT_FORCED_BY_DUMPER") then
                    i(dg[2], 0)
                end
                return unpack(dg)
            end},
        {__index = _G, __newindex = _G, __metatable = false}
    )
    ...
    -- setfenv: In Luau, setfenv on functions is not supported (only level-based)
    rawset(eR, "setfenv", function(f, env)
        ...
        -- In Luau/Roblox, setfenv is not supported on function values
        error("setfenv cannot be applied to a function in Luau", 2)
    ...
    do
        ...
        rawset(eR, "setthreadidentity",    function(id)
            local n = tonumber(id)
            if n == nil or n < 0 then
                error("invalid thread identity value: " .. tostring(id), 2)
            end
            _tid = n
        end)
        rawset(eR, "getidentity",          function() return _tid end)
        rawset(eR, "setidentity",          function(id)
            local n = tonumber(id)
            if n == nil or n < 0 then
                error("invalid thread identity value: " .. tostring(id), 2)
            end
        ...
        rawset(eR, "setthreadcontext",     function(id)
            local n = tonumber(id)
            if n ~= nil then _tid = n end
        end)
    ...
    do
        ...
        rawset(eR, "isreadonly",   function(tbl)
            -- frozen tables (table.isfrozen) are also considered readonly
            if type(tbl) == "table" and table.isfrozen and table.isfrozen(tbl) then return true end
            if _ro[tbl] == true then return true end
            -- Built-in libraries are always readonly in Roblox
            local _em = rawget(eR, "math") or math
            local _es = rawget(eR, "string") or string
            if tbl == _em or tbl == _es or tbl == math or tbl == string then return true end
            if tbl == utf8 or (type(tbl) == "table" and tbl == _G.utf8) then return true end
            return false
        end)
    ...
    do
        -- getfflag always returns "" regardless of setfflag, so new==old→error→pcall fails (anti-dtc36)
        rawset(eR, "setfflag", function(k, v) end)  -- no-op: don't persist
        rawset(eR, "getfflag", function(k) return "" end)  -- always return default
    end
    ...
    do
        ...
        rawset(eR, "iscclosure",    function(f)
            if type(f) ~= "function" then return false end
            if _ccs[f] then return true end  -- newcclosure = C closure
            -- Use debug.getinfo to detect native (C) functions
            local ok_, info_ = pcall(debug.getinfo, f, "S")
            if ok_ and info_ and info_.what == "C" then return true end
            return false
        end)
    ...
    rawset(eR, "hookmetamethod",       function(obj, m_, r_)
        -- hookmetamethod must error (anti-cheat detects working hookmetamethod as executor sign)
        error("hookmetamethod: cannot hook protected metatable", 2)
    end)
    rawset(eR, "replaceclosure",       function(f, r_) if type(r_) == "function" then return r_ end return f end)
    rawset(eR, "islclosure",           function(f)
        if type(f) ~= "function" then return false end
        if _ccs and _ccs[f] then return false end  -- newcclosure = C closure
        local ok_, info_ = pcall(debug.getinfo, f, "S")
        if ok_ and info_ and info_.what == "C" then return false end
        return true
    end)
    rawset(eR, "iscclosure_native",    function(f) return type(f) == "function" and (function()
        local ok_, info_ = pcall(debug.getinfo, f, "S")
        return ok_ and info_ and info_.what == "C"
    end)() end)
    ...
    rawset(eR, "getrawmetatable",      function(x)
        -- Return nil for the sandbox env itself (it has __metatable = false)
        if x == eR then return nil end
        if type(x) == "table" or type(x) == "userdata" then
            local _mt = a.getmetatable(x)
            if _mt == nil then return {} end
            return _mt
        end
        return {}
    end)
    ...
    -- getrenv must return a DIFFERENT table from getgenv (represents Roblox env, not executor env)
    rawset(eR, "getrenv",              function() return _G end)
    ...
    rawset(eR, "getnilinstances",      function()
        -- Return ≥10 nil-parented proxy instances so tests that check #nils >= 10 pass
        local _nils = {}
        for _i = 1, 15 do
            local _np = bj("Part", false)
            t.property_store[_np] = {ClassName = "Part", Name = "NilPart" .. _i}
            _nils[_i] = _np
        end
        return _nils
    end)
    ...
    rawset(eR, "getproto",             function(f, idx) return nil end)
    rawset(eR, "getstack",             function(lvl, idx) return nil end)
    rawset(eR, "setstack",             function(lvl, idx, val) end)
    rawset(eR, "getscriptbytecode",    function(f_)
        -- Return nil for LocalScript instances (real Roblox can't get bytecode of LocalScripts from executor)
        if f_ ~= nil and G(f_) then
            local _ps = t.property_store[f_]
            if _ps and _ps.ClassName == "LocalScript" then
                return nil
            end
        end
        return ""
    ...
    rawset(eR, "setfpscap",            function(v_) _fpscap_sandbox = v_ end)
    rawset(eR, "getfpscap",            function() return _fpscap_sandbox or 60 end)
    ...
    rawset(eR, "getcustomasset",       function(p_) error("getcustomasset: file does not exist", 2) end)
    ...
    do
        local _frozen = _setmetatable({}, {__mode = "k"})
        local _table_ext = _setmetatable({}, {__index = table})
        _table_ext.freeze = function(t_)
            if type(t_) == "table" then
                _frozen[t_] = true
                -- Only apply the full backing-store approach to PLAIN user tables
                -- (no existing metatable). Tables that already have a metatable
                -- (e.g. sandbox wrappers with __index = stdlib) must not have their
                -- metatable replaced, as that would destroy their __index chain.
                -- __newindex is only called for NEW keys, so for plain tables we move
                -- all existing keys to a backing store so every write goes through __newindex.
                local _existing_mt = a.getmetatable(t_)
                if not _existing_mt then
                    local _backing = {}
                    local _keys = {}
                    for k in next, t_ do _keys[#_keys+1] = k end
                    for _, k in ipairs(_keys) do
                        _rawset(_backing, k, _rawget(t_, k))
                        _rawset(t_, k, nil)
                    end
                    local _freeze_mt = {}
                    _rawset(_freeze_mt, "__index",    _backing)
                    _rawset(_freeze_mt, "__newindex", function(_, k, v)
                        error("attempt to modify a frozen table", 2)
                    end)
                    _rawset(_freeze_mt, "__len",   function() return #_backing end)
                    _rawset(_freeze_mt, "__pairs", function() return next, _backing, nil end)
                    _setmetatable(t_, _freeze_mt)
                else
                    -- Table already has a metatable: just install __newindex guard.
                    -- This won't block writes to existing keys (Lua limitation), but
                    -- it marks the table as frozen and blocks writes to new keys.
                    if type(_existing_mt) == "table" and not _rawget(_existing_mt, "__newindex") then
                        _rawset(_existing_mt, "__newindex", function(_, k, v)
                            error("attempt to modify a frozen table", 2)
                        end)
                    end
                end
        ...
        -- Guard table.insert against unbounded growth (OOM protection)
        local _orig_tinsert = table.insert
        _table_ext.insert = function(t_, ...)
            if type(t_) == "table" then
                local _sz = rawlen(t_)
                if _sz >= r.MAX_TABLE_ENTRIES then
                    error("TIMEOUT_FORCED_BY_DUMPER: table size limit exceeded", 2)
                end
            end
            return _orig_tinsert(t_, ...)
        end
        _table_ext.isfrozen = function(t_)
            -- Check both the sandbox-local registry AND the global registry
            -- (stdlib tables are marked frozen via global table.freeze).
            return _frozen[t_] == true or table.isfrozen(t_)
        end
    ...
    do
        ...
        -- Override random to explicitly error on empty interval
        _math_ext.random = setmetatable({}, {
            __call = function(_, m, n, ...)
                if m ~= nil and n ~= nil
                    and type(m) == "number" and type(n) == "number"
                    and m > n then
    ...
    -- string override: per-call cap + cumulative budget to stop OOM bombs
    do
        local _string_ext = setmetatable({}, {__index = string})
        _string_ext.rep = function(s, n, sep)
            local _s = tostring(s or "")
            local _n = math.max(0, math.floor(tonumber(n) or 0))
            local _sep = tostring(sep or "")
            local _unit = #_s + ((_n > 1) and #_sep or 0)
            local _est = _unit * _n
            -- Per-call hard cap (1 MB)
            if _est > r.MAX_STRING_REP_BYTES then
                _n = math.max(1, math.floor(r.MAX_STRING_REP_BYTES / math.max(1, _unit)))
                _est = _unit * _n
            end
            -- Cumulative sandbox budget (50 MB total per execution)
            local _cur = (_G._CATMIO_EXEC_ALLOC or 0) + _est
            if _cur > r.MAX_EXEC_TOTAL_ALLOC then
                error("TIMEOUT_FORCED_BY_DUMPER: string allocation budget exceeded", 2)
            end
            _G._CATMIO_EXEC_ALLOC = _cur
            return _orig_string_rep(_s, _n, sep)
        end
        rawset(eR, "string", _string_ext)
    end
    -- Separate _G from getfenv() so that _G writes don't leak into the function env
    -- (anti-detection: `getfenv().key` should be nil even after `_G.key = val`)
    do
        local _user_G_store = {}
        local _user_G = setmetatable({}, {
            __index = function(_, k)
                local _uv = rawget(_user_G_store, k)
                if _uv ~= nil then return _uv end
                return rawget(eR, k)
            end,
            __newindex = function(_, k, v)
                rawset(_user_G_store, k, v)
            end,
        })
        rawset(eR, "_G", _user_G)
    end
    ...
    rawset(eR, "debug", {
        ...
        getinfo      = function(f, what)
            ...
            return {source="=[C]",what="C",name="unknown",short_src="[C]",currentline=-1,nups=0,nparams=0,isvararg=true}
        end,
        getproto     = function(f, idx, copy)
            -- Empty Lua functions have no sub-protos; return nil to reflect real behavior
            return nil
        end,
        ...
        traceback    = function(msg, lvl)
            local _tb = original_traceback(tostring(msg or ""), (tonumber(lvl) or 1) + 1)
            return _tb
        end,
        profilebegin = function() end,
        profileend   = function() end,
        sethook      = function() end,
        gethook      = function() return nil end,
        -- debug.info: Luau-style function info query
        -- format "a" returns numparams, isvararg (both as Lua values)
        -- format "s" returns short_src string
        info         = function(f, fmt)
            if type(f) == "number" then
                -- Integer level: map sandbox level i → real level i+1 (skip the info wrapper itself)
                local n = math.floor(f)
                if n < 0 then return nil end
                local di = debug.getinfo(n + 1, "f")
                if not di then return nil end
                if fmt == "f" then
                    return di.func  -- truthy if a frame exists, nil otherwise
                elseif fmt == "l" then
                    local di2 = debug.getinfo(n + 1, "l")
                    return di2 and di2.currentline or 0
                elseif fmt == "s" then
                    local di2 = debug.getinfo(n + 1, "S")
                    return di2 and (di2.short_src or "?") or "?"
                elseif fmt == "a" then
                    local di2 = debug.getinfo(n + 1, "u")
                    if di2 then
                        local nparams = di2.nparams or 0
                        local isvararg = (di2.isvararg == 1 or di2.isvararg == true)
                        return nparams, isvararg
                    end
                    return 0, false
                else
                    local di2 = debug.getinfo(n + 1, fmt or "nSl")
                    return di2
                end
            end
            if type(f) ~= "function" then return nil end
            if fmt == "a" then
                local ok2, di = pcall(debug.getinfo, f, "u")
                if ok2 and di then
                    local nparams = di.nparams or 0
                    local isvararg = (di.isvararg == 1 or di.isvararg == true)
                    return nparams, isvararg
                end
                return 0, false
            elseif fmt == "s" then
                local ok2, di = pcall(debug.getinfo, f, "S")
                if ok2 and di then return di.short_src or "?" end
                return "?"
            else
                local ok2, di = pcall(debug.getinfo, f, fmt or "nSl")
                if ok2 and di then return di end
                return nil
            end
        end,
    })
    ...
    rawset(eR, "getscriptbytecode",function(f_)
        if f_ ~= nil and G(f_) then
            local _ps = t.property_store[f_]
            if _ps and _ps.ClassName == "LocalScript" then return nil end
        end
        return ""
    end)
    rawset(eR, "getscripthash",  function() return string.rep("0", 64) end)
    ...
    rawset(eR, "identifyexecutor",function() return "ExploitExecutor", "1.0" end)
    rawset(eR, "getexecutorname", function() return "ExploitExecutor" end)
    rawset(eR, "hookmetamethod",  function(obj, method, hook)
        error("hookmetamethod: cannot hook protected metatable", 2)
    end)
    ...
    -- version() — Luau-style global returning the engine version string
    rawset(eR, "version", function() return "0.587.0.6438153" end)
    -- Explicitly nil executor-brand globals so anti-cheat checks like
    -- `if syn or fluxus or krnl or scriptware then` do NOT fire
    rawset(eR, "syn",        nil)
    rawset(eR, "fluxus",     nil)
    rawset(eR, "krnl",       nil)
    rawset(eR, "scriptware", nil)
    rawset(eR, "sentinel",   nil)
    rawset(eR, "oxygen",     nil)
    rawset(eR, "proto",      nil)
    -- _VERSION must be "Luau" in the sandbox (Roblox/Luau engine identifier)
    rawset(eR, "_VERSION",   "Luau")
    -- _byp_require signals that the sandbox's require() is the custom one
    rawset(eR, "_byp_require", true)
    -- gethwid — hardware identifier stub (exploit executor API)
    rawset(eR, "gethwid", function() return "CATMIO-HWID-" .. string.rep("A", 32) end)
    -- xpcall override: track when error handler is executing so task.wait() can error
    do
        local _in_errhandler = 0
        local _real_xpcall = xpcall
        rawset(eR, "xpcall", function(f, errhandler, ...)
            local _wh = function(msg)
                _in_errhandler = _in_errhandler + 1
                local ok2, r2 = pcall(errhandler, msg)
                _in_errhandler = _in_errhandler - 1
                if not ok2 then error(r2, 0) end
                return r2
            end
            return _real_xpcall(f, _wh, ...)
        end)
        -- Wrap task so task.wait() errors inside an error handler
        local _task_orig = task
        local _task_wrapped = setmetatable({}, {__index = _task_orig})
        _task_wrapped.wait = function(dt)
            if _in_errhandler > 0 then
                error("cannot yield inside error handler", 2)
            end
            return _task_orig.wait(dt)
        end
        rawset(eR, "task", _task_wrapped)
    end
    -- Freeze stdlib tables so table.isfrozen() returns true for all of them
    -- NOTE: must run AFTER xpcall block so _task_wrapped is already rawset into eR
    do
        local _tbl = rawget(eR, "table")
        if _tbl and _tbl.freeze then
            -- Use the global LIGHT freeze for stdlib/infrastructure tables.
            -- The heavy backing-store sandbox freeze is reserved for user-created
            -- plain tables; applying it to string/coroutine/bit32 breaks internal
            -- Lua method dispatch because rawget on the backed table returns nil.
            local function _try_freeze(t_)
                if type(t_) == "table" then pcall(table.freeze, t_) end
            end
            _try_freeze(rawget(eR, "math"))
            _try_freeze(_tbl)
            _try_freeze(rawget(eR, "os"))
            _try_freeze(string)
            _try_freeze(coroutine)
            _try_freeze(bit32)
            -- Freeze the sandbox's task (_task_wrapped), not the global one
            _try_freeze(rawget(eR, "task"))
            local _smeta = getmetatable("")
            if type(_smeta) == "table" then
                _try_freeze(_smeta)
                if type(_smeta.__index) == "table" then
                    _try_freeze(_smeta.__index)
                end
            end
        end
    end
    ...
    -- Reset per-execution allocation budget (string.rep budget tracking)
    _G._CATMIO_EXEC_ALLOC = 0
    _G._CATMIO_EXEC_BUDGET = r.MAX_EXEC_TOTAL_ALLOC
